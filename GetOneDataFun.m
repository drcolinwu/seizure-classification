...
for i=1:length(NormalTag)
    ...
    for j=1:length(name)
        if exist('ColumNumber','var') == 1
            eval([name{j},'_done_',NormalTag{i},'=dsS(',name{j},'1024_',NormalTag{i},',ColumNumber(i));']);
        else
            eval([name{j},'_done_',NormalTag{i},'=dsS(',name{j},'1024_',NormalTag{i},',NewColunm);']);
        end
...
%             name{i},'_Fil_Seizure;']);
        ...
        eval(['[',name{i},', lable, ]=DivideData3Kind_combine(',name{i},...
            '_Fil_Control, ',name{i},'_Fil_Preictal, ',...
            name{i},'_Fil_Seizure);']);
